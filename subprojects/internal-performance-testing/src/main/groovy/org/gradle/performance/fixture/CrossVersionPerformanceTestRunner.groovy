...
class CrossVersionPerformanceTestRunner extends PerformanceTestSpec {
    ...
    CrossVersionPerformanceResults run() {
        ...
        baselineVersions.each { baselineVersion ->
            runVersion("baseline", buildContext.distribution(baselineVersion.version), perVersionWorkingDirectory(baselineVersion.version, maxWorkingDirLength), baselineVersion.results)
        }
    ...
    private void runVersion(String role, GradleDistribution dist, File workingDir, MeasuredOperationList results) {
        def gradleOptsInUse = resolveGradleOpts()
        def builder = GradleBuildExperimentSpec.builder()
            .projectName(testProject)
            .displayName("${role} build ${dist.version.version}")
